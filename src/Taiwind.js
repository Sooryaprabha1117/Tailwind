...
const gradients = [
  'linear-gradient(to right, #FF5733, #FFC300)', // First gradient
...
function ColorPicker() {
  ...
  const [previousGradient, setPreviousGradient] = useState(gradients[0]);

  const handleGradientClick = (gradient) => {
    setPreviousGradient(selectedGradient); // Store current gradient as the previous one
    setSelectedGradient(gradient);         // Set the new gradient
  };

  const handleReset = () => {
    setSelectedGradient(previousGradient); // Revert to the previous gradient
  };

  const handleRandomGradient = () => {
    setPreviousGradient(selectedGradient); // Store current gradient as the previous one
    const randomIndex = Math.floor(Math.random() * gradients.length);
    setSelectedGradient(gradients[randomIndex]); // Set a new random gradient
  };
